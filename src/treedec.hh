...
inline int treewidth(const Graph& G) {
    auto set_union_vec = [](const std::vector<int>& A, const std::vector<int>& B) {
        std::vector<int> C;
        std::set_union(A.begin(), A.end(), B.begin(), B.end(), std::back_inserter(C));
        return C;
    };
    ...
    for (int u = 0; u < n; ++u) {
        nbh[u].push_back(u);
        std::sort(nbh[u].begin(), nbh[u].end());
        nbh[u].erase(std::unique(nbh[u].begin(), nbh[u].end()), nbh[u].end());
    }
    std::priority_queue<std::pair<int,int>, std::vector<std::pair<int,int>>, std::greater<std::pair<int,int>>> pq;
    for (int u = 0; u < n; ++u) pq.emplace(nbh[u].size(), u);

    int tw = 0;
    while (!pq.empty()) {
        auto [deg, u] = pq.top(); pq.pop();
        if (deg != (int)nbh[u].size()) continue;
        tw = std::max(tw, deg - 1); // bag-size − 1
        for (int v : nbh[u]) if (v != u) {
            auto U = set_union_vec(nbh[u], nbh[v]);
            U.erase(std::remove(U.begin(), U.end(), u), U.end());
            U.swap(nbh[v]);
            if (nbh[v].size() < U.size())
                pq.emplace(nbh[v].size(), v);
        }
    }
    return tw;
}
